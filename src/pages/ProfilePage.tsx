...
const mockOrders: Order[] = [
  ...
  },
  {
    ...
    amount: 35000,
    deliveryFee: 1500,
    items: [{ name: "Anniversary Hamper", type: "Luxury Set", price: 35000 }],
  },
  {
    id: "#I1245681",
    ...
    amount: 45000,
    deliveryFee: 1500,
    items: [{ name: "Corporate Gift Pack", type: "Business", price: 45000 }],
  },
];
...
export function ProfilePage() {
  ...
  // Update profile handler with Supabase integration
  ...
  // Password change handler with Supabase integration
  const handlePasswordChangeAsync = async () => {
    // Validation
  ...
  useEffect(() => {
    if (isEditing) {
      const inputs =
        document.querySelectorAll<HTMLInputElement>(".profile-input");
      if (inputs.length > 0) {
        inputs[0].focus();
      }
    }
  ...
  const PasswordToggle = ({
    ...
    <button style={passwordToggleStyle} onClick={onClick} type="button">
      {show ? (
  ...
  const handleCancelOrder = async (orderId: string) => {
    try {
      ...
      const order = orders.find(
        (o) => o.orderNumber === orderId.replace("#", ""),
      );
      if (order) {
    ...
    <>
          ...
          {isEditing ? (
            <div style={{ display: "flex", gap: "12px" }}>
              ...
              <button
                style={{
                  ...editLinkStyle,
                  color: hasChanges ? "#FF8C42" : "#9CA3AF",
                }}
      ...
      <div style={dangerZoneStyle}>
        ...
        <p style={dangerDescriptionStyle}>
          Deleting your account is not reversible and all data including order
          history will be erased
        </p>
        <button style={deleteButtonStyle}>
          <Trash2 size={18} />I understand, delete my account
        </button>
      </div>
    </>
  );
      ...
      <div style={filterBarStyle}>
        ...
        <div style={filterDropdownsStyle}>
          <div
            ...
            onClick={() =>
              setStatusFilter(
                statusFilter === "All Orders" ? "Delivered" : "All Orders",
              )
            }
          >
            ...
            <div key={order.id} style={orderCardStyle}>
              ...
              <div style={financialSectionStyle}>
                <div style={financialRowStyle}>
                  ...
                  <span style={financialValueStyle}>
                ...
                <div style={totalRowStyle}>
                  ...
                  <span style={totalValueStyle}>
                    ₦{(order.amount + order.deliveryFee).toLocaleString()}
                  </span>
                </div>
              </div>
    ...
    <>
      ...
      {recentlyViewedProducts.length > 0 ? (
        ...
        <div style={recentlyViewedGridStyle}>
          {recentlyViewedProducts.map((product: (typeof allProducts)[0]) => (
            <div
              key={product.id}
  ...
  // State for saved addresses - initially empty
  const [savedAddresses, setSavedAddresses] = useState<AddressCardData[]>([]);
  ...
  const handleNotificationToggle = (key: keyof typeof notificationToggles) => {
    ...
    const keyString = String(key)
      ...
      .trim();
    toast.success(
      `${keyString} ${!notificationToggles[key] ? "enabled" : "disabled"}`,
    );
  };
    ...
    <div key={card.id} style={addressCardStyle}>
      ...
      <div style={addressCardActionsStyle}>
        {card.isDefault ? (
          <>
            <button
              style={settingsEditLinkStyle}
              onClick={() => toast.info("Editing address...")}
            >
              <Pencil size={16} />
              Edit
            </button>
            <button
              ...
              onClick={() => toast.warning("Remove address clicked")}
            >
              <Trash2 size={16} />
              Remove
            </button>
    ...
    <>
      ...
      {savedAddresses.length > 0 ? (
        <div style={addressCardContainerStyle}>
          {savedAddresses.map((card, index) => renderAddressCard(card, index))}
        </div>
      ) : (
        <div
          style={{
